...
import { Sparkles, Plus, Settings2, Trash2, Loader2, MessageSquare, Bot, Zap, Image as ImageIcon, Mic, Globe } from "lucide-react";
            ...
            <div>
              <p className="font-heading font-bold text-base flex items-center gap-1.5">
                ...
                <Badge variant="secondary" className="text-xs gap-1">
                  <Sparkles className="w-2.5 h-2.5" /> {MODELS.find((m) => m.value === selectedModel)?.label || "Auto"}
                </Badge>
                {webSearch && <Badge className="text-xs gap-1"><Globe className="w-2.5 h-2.5" /> Web</Badge>}
              </p>
              <p className="text-xs text-muted-foreground">
                {activeId ? `${messages.length} messages` : "Image, voice, agent system, multi-model"}
              </p>
            </div>
          </div>
            ...
            <SheetContent className="w-[500px] sm:w-[540px] overflow-auto">
              ...
              <div className="p-4 space-y-4">
                <p className="text-sm text-muted-foreground">
                  These instructions control how Xtreme GPT behaves. Edit them to customize the AI's personality, capabilities, and rules.
                </p>
                <textarea
        ...
        <div ref={scrollRef} className="flex-1 overflow-auto p-4 space-y-4">
          {!activeId ? (
            <div className="flex flex-col items-center justify-center h-full text-center">
              <div className="w-16 h-16 rounded-2xl bg-gradient-to-br from-primary to-primary/70 flex items-center justify-center mb-4">
                <Zap className="w-8 h-8 text-primary-foreground" />
              </div>
              <h2 className="text-2xl font-heading font-bold mb-2">Xtreme GPT</h2>
              <p className="text-sm text-muted-foreground max-w-md mb-6">
                The most powerful AI shell — image generation, voice input/output, web search, multi-model support, and full access to the Xtreme platform.
              </p>
              <div className="grid grid-cols-2 gap-3 max-w-lg">
                ...
                <Card><CardContent className="p-4 text-left"><ImageIcon className="w-5 h-5 text-primary mb-2" /><p className="text-sm font-medium">Image Generation</p><p className="text-xs text-muted-foreground">Switch to Image mode to create AI images</p></CardContent></Card>
                <Card><CardContent className="p-4 text-left"><Mic className="w-5 h-5 text-primary mb-2" /><p className="text-sm font-medium">Voice I/O</p><p className="text-xs text-muted-foreground">Speak to the AI, hear it read responses aloud</p></CardContent></Card>
                <Card><CardContent className="p-4 text-left"><Globe className="w-5 h-5 text-primary mb-2" /><p className="text-sm font-medium">Web Search</p><p className="text-xs text-muted-foreground">Let the AI search the internet for context</p></CardContent></Card>
              </div>
              <Button onClick={createConversation} className="mt-6 gap-2" size="lg">
            ...
            <>
              ...
              {busy && (
                <div className="flex items-center gap-2 text-sm text-muted-foreground">
                  <Loader2 className="w-4 h-4 animate-spin" /> Xtreme GPT is working...
                </div>
              )}
            </>
          )}
