...
export default async function (req: Request): Promise<Response> {
  ...
  try {
    ...
    const validatorAgentId = isNonEmptyString(body.validator_agent_id) ? body.validator_agent_id.trim() : "external-validator";
    const implementationAgentId = isNonEmptyString(body.implementation_agent_id) ? body.implementation_agent_id.trim() : "";
    const observedAt = isNonEmptyString(body.observed_at) ? body.observed_at.trim() : nowIso();
    ...
    if (ageMs(observedAt) > MAX_EVIDENCE_AGE_MS) errors.push("observed_at is stale, invalid, or future-dated beyond allowed clock skew");
    ...
    const hasEvidence = Boolean(
      ...
      evidence.artifact_urls.length > 0
    );
    ...
    const recent = await base44.asServiceRole.entities.EvidenceReceipt.list("-timestamp", 500).catch(() => []);
    ...
    const created = await base44.asServiceRole.entities.EvidenceReceipt.create({
      ...
      data: {
        ...
        score: numericScore,
        failure_fingerprint: isNonEmptyString(body.failure_fingerprint) ? body.failure_fingerprint : "",
